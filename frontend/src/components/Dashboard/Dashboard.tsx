...
const Dashboard = () => {
  const { latest, analytics, actuators, hours, setHours, load } = useDashboard();

  const statCards  = latest && analytics ? getStatCards(latest, analytics) : [];
  ...
  return (
    <div>
      ...
      <div className={styles.actuatorsRow}>
        <ActuatorCard type="light" state={actuators?.["light-ctrl"]} />
        <ActuatorCard type="alarm" state={actuators?.["alarm-relay"]} />
      </div>
