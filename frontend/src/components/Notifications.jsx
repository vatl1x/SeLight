    ...
    <div>
      ...
      <div className="notif-list">
        {notifs.map(n => {
          ...
          return (
            <div key={n.id} className={`notif-item ${n.read ? 'notif-read' : ''} ${meta.cls}`}>
              <span className="notif-icon">{meta.icon}</span>
              <div className="notif-body">
                <div className="notif-msg">{n.message}</div>
                <div className="notif-meta">
                  {n.device_id && <span className="notif-device">{n.device_id}</span>}
                  <span>{new Date(n.timestamp).toLocaleString('ru-RU')}</span>
                </div>
              </div>
              {!n.read && (
